...
public class PendingTripsFragment extends Fragment implements View.OnClickListener, IUpdatableFragment, OnTripListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        AnalyticsUtility.logEventOpen(AnalyticsUtility.Events.PENDING_TRIPS);
    ...
    private void loadDate() {
        ...
        call.enqueue(new Callback<ModelBookingAPI>() {
            ...
            public void onResponse(@NonNull Call<ModelBookingAPI> call, @NonNull Response<ModelBookingAPI> response) {
                ...
                if (getActivity() == null)
                    return;
                if (PendingTripsFragment.this.getContext() == null)
                    return;
                if (!isAdded())
                    return;
                ...
                if (response.code() != 200) {
                    try {
                        ...
                        GlobalInfoDialog.getInstance(PendingTripsFragment.this.getContext()).setTitle(getString(R.string.error)).setMessage(message).show();
                        return;
                ...
                if (body == null) {
                    GlobalInfoDialog.getInstance(PendingTripsFragment.this.getContext()).setTitle(getString(R.string.error)).setMessage(getString(R.string.please_check_your_internet_connection)).show();
                    return;
                }
                if (body.getStatus() == 200) {
                    AnalyticsUtility.logEventAPISuccess(AnalyticsUtility.Events.PENDING_TRIPS);
                    if (body.getData().getPendding().size() == 0) {
                        binding.layoutNoTrips.getRoot().setVisibility(View.VISIBLE);
                        UtilsPreference.getInstance(getActivity()).savePreference(Constants.IS_BOOKINGS_PENDING, false);
                    } else {
                        BookingAdapter bookingAdapter = new BookingAdapter(PendingTripsFragment.this.getContext(), body.getData().getPendding(), PendingTripsFragment.this, true);
                        ...
                        UtilsPreference.getInstance(getActivity()).savePreference(Constants.IS_BOOKINGS_PENDING, true);
                    }
            ...
            public void onFailure(@NonNull Call<ModelBookingAPI> call, @NonNull Throwable t) {
              ...
              //  GlobalInfoDialog.getInstance(PendingTripsFragment.this.getContext()).setTitle(getString(R.string.error)).setMessage(PendingTripsFragment.this.getContext().getResources().getString(R.string.cant_connect)).show();
                binding.layoutNoTrips.getRoot().setVisibility(View.GONE);
