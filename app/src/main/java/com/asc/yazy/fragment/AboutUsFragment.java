...
import com.asc.yazy.databinding.FragmentAboutUsBinding;
...
public class AboutUsFragment extends Fragment implements View.OnClickListener, IFragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        binding = DataBindingUtil.inflate(LayoutInflater.from(getContext()), R.layout.fragment_about_us, null, false);
    ...
    private void loadDate() {
        ...
        Call<ModelStatic> call = mApiService.aboutUs(Constants.getLANGUAGE());
        call.enqueue(new Callback<ModelStatic>() {
            ...
            public void onResponse(@NonNull Call<ModelStatic> call, @NonNull Response<ModelStatic> response) {
                ...
                if (response.code() != 200) {
                    try {
                        ...
                        GlobalInfoDialog.getInstance(AboutUsFragment.this.getContext()).setTitle(getString(R.string.error)).setMessage(message).show();
                        return;
                ...
                if (body == null) {
                    GlobalInfoDialog.getInstance(AboutUsFragment.this.getContext()).setTitle(getString(R.string.error)).setMessage(getString(R.string.please_check_your_internet_connection)).show();
                    return;
                }
                if (body.getStatus() == 200) {
                    AnalyticsUtility.logEventAPISuccess(AnalyticsUtility.Events.ABOUT);
                    binding.loader.stopShimmerAnimation();
                    binding.tvAbout.setBackground(null);
                    binding.tvAbout.loadDataWithBaseURL("", body.getData().getPageContent(), "text/html", "UTF-8", "");
                } else {
                    GlobalInfoDialog.getInstance(AboutUsFragment.this.getContext()).setTitle(getString(R.string.error)).setMessage(body.getMessage()).show();
                }
            }
            ...
            public void onFailure(@NonNull Call<ModelStatic> call, @NonNull Throwable t) {
                ...
                //  GlobalInfoDialog.getInstance(AboutUsFragment.this.getContext()).setTitle(getString(R.string.error)).setMessage(AboutUsFragment.this.getContext().getResources().getString(R.string.cant_connect)).show();
