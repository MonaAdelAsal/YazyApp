...
public class ConfirmedTripsFragment extends Fragment implements View.OnClickListener, IUpdatableFragment, OnTripListener, SwipeRefreshLayout.OnRefreshListener {


    private FragmentMyTripsBinding binding;

    public ConfirmedTripsFragment() {
    }
    ...
    private void loadDate() {
        ...
        call.enqueue(new Callback<ModelBookingAPI>() {
            ...
            public void onResponse(@NonNull Call<ModelBookingAPI> call, @NonNull Response<ModelBookingAPI> response) {
                ...
                if (getActivity() == null)
                    return;
                if (ConfirmedTripsFragment.this.getContext() == null)
                    return;
                if (!isAdded())
                    return;
                ...
                if (response.code() != 200) {
                    try {
                        ...
                        GlobalInfoDialog.getInstance(ConfirmedTripsFragment.this.getContext()).setTitle(getString(R.string.error)).setMessage(message).show();
                        return;
                ...
                if (body == null) {
                    GlobalInfoDialog.getInstance(ConfirmedTripsFragment.this.getContext()).setTitle(getString(R.string.error)).setMessage(getString(R.string.please_check_your_internet_connection)).show();
                    return;
                }
                if (body.getStatus() == 200) {
                    AnalyticsUtility.logEventAPISuccess(AnalyticsUtility.Events.CONFIRMED_TRIPS);
                    if (body.getData().getCompleted().size() == 0) {
                        binding.layoutNoTrips.getRoot().setVisibility(View.VISIBLE);
                    } else {
                        BookingAdapter bookingAdapter = new BookingAdapter(ConfirmedTripsFragment.this.getContext(), body.getData().getCompleted(), ConfirmedTripsFragment.this, false);
            ...
            public void onFailure(@NonNull Call<ModelBookingAPI> call, @NonNull Throwable t) {
              ...
              //  GlobalInfoDialog.getInstance(ConfirmedTripsFragment.this.getContext()).setTitle(getString(R.string.error)).setMessage(ConfirmedTripsFragment.this.getContext().getResources().getString(R.string.cant_connect)).show();
                binding.layoutNoTrips.getRoot().setVisibility(View.GONE);
