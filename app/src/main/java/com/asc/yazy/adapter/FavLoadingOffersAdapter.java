...
import com.asc.yazy.databinding.OfferGridCell2Binding;
import com.asc.yazy.interfaces.OnOfferItemListener;

import java.util.List;

public class FavLoadingOffersAdapter extends RecyclerView.Adapter<FavLoadingOffersAdapter.ViewHolder> {

    private final List<ModelOffer> offersList;
    private final OnOfferItemListener listener;
    private final boolean isLoading;

    public FavLoadingOffersAdapter(List<ModelOffer> offersList, OnOfferItemListener listener, boolean isLoading) {
    ...
    @NonNull
    @Override
    public FavLoadingOffersAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        OfferGridCell2Binding binding = DataBindingUtil.inflate(LayoutInflater.from(parent.getContext()), R.layout.offer_grid_cell_2, parent, false);
        binding.setListener(listener);
        return new FavLoadingOffersAdapter.ViewHolder(binding);
    }

    @Override
    public void onBindViewHolder(@NonNull FavLoadingOffersAdapter.ViewHolder holder, int position) {
    ...
    @Override
    public void onViewAttachedToWindow(@NonNull FavLoadingOffersAdapter.ViewHolder holder) {
        super.onViewAttachedToWindow(holder);
        holder.bind();
    }

    @Override
    public void onViewDetachedFromWindow(@NonNull FavLoadingOffersAdapter.ViewHolder holder) {
    ...
    class ViewHolder extends RecyclerView.ViewHolder {
        private OfferGridCell2Binding binding;

        ViewHolder(OfferGridCell2Binding binding) {
